...
| `feature_extractor.py` | Extraction de caractéristiques comportementales par fenêtre temporelle glissante (1 min pour les rafales/scans, 5 min pour les tendances). Produit 16 colonnes, dont 2 identifiants (`agent_name`, `window_start`) et **14 caractéristiques numériques** : `window_minutes`, `event_count`, `unique_dest_ips`, `unique_dest_ports`, `distinct_rule_ids`, `avg_rule_level`, `max_rule_level`, `suricata_ratio`, `time_span_seconds`, et les variantes `inbound_*`/`outbound_*`. **Sur ces 14, seules 11 sont exploitables** : les trois `inbound_*` sont structurellement nulles (voir section 8). La distinction entrant/sortant annoncée comme point de conception ne fonctionne pas en pratique |
...
- **Écart architectural majeur — le moteur IA n'a jamais tourné sur des données live.** `feature_extractor.py` produit 14 caractéristiques numériques agrégées par fenêtre temporelle (`event_count`, `unique_dest_ports`, etc. — dont 11 seulement sont exploitables, voir plus bas), tandis que les modèles sont entraînés sur le schéma NSL-KDD (41 colonnes détaillées par session). Aucun recouvrement. Toute validation ML (rapports, dashboard, démonstrations) porte exclusivement sur NSL-KDD. `feature_schema.py` (15/08/2026) empêche un échec silencieux ou cryptique, mais ne résout pas l'écart.
...
- **Cause racine unique : le pipeline lit des *alertes*, pas des *flux*.** Trois défauts jusqu'ici décrits séparément — l'écart de schéma NSL-KDD, la mort des caractéristiques `inbound_*`, et la couverture partielle de `alerts.jsonl` — sont trois symptômes d'un même choix d'architecture. La chaîne `collector.py` → `normalizer.py` → `feature_extractor.py` s'alimente à l'index `wazuh-alerts-*`, or Wazuh n'indexe que les événements Suricata de type `alert`. Tout ce qui n'a pas déclenché de règle est invisible en aval, quelle que soit la qualité du code de features.

  **Conséquence 1 — les caractéristiques `inbound_*` sont mortes.** Mesuré le 06/09/2026 sur 10 627 alertes / 315 fenêtres d'une minute : `inbound_event_count`, `inbound_unique_src_ips` et `inbound_unique_ports` sont à zéro sur **315 fenêtres sur 315**, tandis que `outbound_event_count` est non nul sur **131** fenêtres (max 243, total 4693). Les alertes indexées portent très majoritairement du trafic sortant de la machine surveillée (anomalies QUIC/TLS, retransmissions). Le jeu de caractéristiques effectif compte donc **11 variables utiles, non 14**, et la distinction entrant/sortant — présentée comme un point de conception dans le journal du 29/07 — ne fonctionne pas en pratique.

  **Conséquence 2 — l'information manquante existe, mais hors d'atteinte.** Le trafic entrant est parfaitement présent dans les enregistrements `event_type: flow` de `eve.json` : **8684 flux TCP entrants** vers la machine surveillée mesurés sur la seule fenêtre du scan du 31/08 (23:45–23:49). Aucun de ces flux n'atteint le pipeline, car aucun n'est un événement `alert`. Ce n'est donc pas un manque de données mais un manque de *chemin* vers les données — ce qui désigne aussi le correctif : brancher la collecte sur `eve.json` plutôt que sur l'index d'alertes. C'est exactement ce que fait `flow_feature_extractor.py` (voir chantier NetFlow ci-dessus), qui reste valide indépendamment de l'échec du transfert inter-domaine.

- **La collecte de `alerts.jsonl` est manuelle et non continue.** `collector.py` est un tirage de 500 alertes par exécution, pas un service. Son checkpoint est resté figé au 29/07/2026 pendant 39 jours ; il est au 14/08/2026 au moment de la rédaction, alors que l'indexeur contient des alertes jusqu'à aujourd'hui. Couverture réelle du fichier au 06/09/2026 : 10 627 alertes du 23/07 au 14/08. **Le retard n'est volontairement pas rattrapé** : la période 29/07–15/08 est dominée à **87,5 % par du bruit QUIC** (9297 alertes « SURICATA QUIC failed decrypt » sur 10 627), précisément la nuisance supprimée à la source le 15/08 (commit `82a88c4`). Rattraper gonflerait le volume sans ajouter de signal. La couverture est documentée telle qu'elle est plutôt que présentée comme continue.
